...
set (not_required_str "OPENMS_INSTALL_DIR;NUMBER_THREADS;CTEST_SITE;CTEST_COVERAGE_COMMAND;BUILD_TYPE;QT_ROOT;QT_WEBKIT_ROOT;OPENMS_TARGET_ARCH;SIGNING_IDENTITY;SIGNING_EMAIL;SIGNING_KEYCHAIN;PYTHON_EXECUTABLE")
## Set non-required boolean variables to "Off" if not present
set (not_required_bool "CDASH_SUBMIT;ENABLE_PREPARE_KNIME_PACKAGE;ENABLE_TOPP_TESTING;ENABLE_CLASS_TESTING;ENABLE_PIPELINE_TESTING;PACKAGE_TEST;BOOST_USE_STATIC;EXTERNAL_CODE_TESTS;OPENMS_COVERAGE;ENABLE_STYLE_TESTING;PYOPENMS;RUN_CHECKER;RUN_PYTHON_CHECKER;BUILD_DOCU;PY_NUM_THREADS;PY_NUM_MODULES;PY_NO_OPTIMIZATION;PY_MEMLEAK_DISABLE;PY_SINGLE_THREADED;ENABLE_UNITYBUILD;WITH_MASCOT_TEST;WITH_MSFRAGGER_TEST;IWYU")
...
set(CMAKE_PREFIX_PATH "$ENV{QT_ROOT};$ENV{QT_WEBKIT_ROOT};$ENV{OPENMS_BREW_FOLDER}")
...
if(NOT DEFINED ENV{SEARCH_ENGINES_DIRECTORY})
  ...
  if("$ENV{ENABLE_TOPP_TESTING}" STREQUAL "ON")
      ...
      FOREACH(subdir ${SUBDIRS})
        ADD_DIR_TO_PATH("$ENV{SEARCH_ENGINES_DIRECTORY}/${subdir}")
      ENDFOREACH()
      ADD_DIR_TO_PATH("${JAVA_BIN_DIR}")

      if (DEFINED ENV{PRIVATE_THIRDPARTY_DIRECTORY})
        SUBDIRLIST(PRIVATE_SUBDIRS $ENV{PRIVATE_THIRDPARTY_DIRECTORY})
          FOREACH(subdir ${PRIVATE_SUBDIRS})
            ADD_DIR_TO_PATH("$ENV{PRIVATE_THIRDPARTY_DIRECTORY}/${subdir}")
          ENDFOREACH()
      endif()    
  endif()
endif()
...
# enable/disable SIRIUS tests
SET(INITIAL_CACHE "${INITIAL_CACHE}
    ENABLE_SIRIUS_TEST:BOOL=ON
")
...
## Add only if present
set (additional_bool_vars ENABLE_USAGE_STATISTICS ENABLE_TOPP_TESTING ENABLE_SIRIUS_TEST ENABLE_CLASS_TESTING ENABLE_STYLE_TESTING WITH_GUI WITH_MASCOT_TEST WITH_MSFRAGGER_TEST DISABLE_WAVELET2DTEST PYOPENMS OPENMS_COVERAGE ADDRESS_SANITIZER CLANG_TIME_TRACE)
...
if("$ENV{IWYU}" STREQUAL "ON")
      SET(INITIAL_CACHE "${INITIAL_CACHE}
        CMAKE_CXX_INCLUDE_WHAT_YOU_USE=/usr/bin/iwyu
      ")
endif()
...
if(WIN32)
  ...
  SET(INITIAL_CACHE "${INITIAL_CACHE}
    BOOST_USE_STATIC=$ENV{BOOST_USE_STATIC}
  " )
  if(APPLE)
    SET(INITIAL_CACHE "${INITIAL_CACHE}
