if [[ -z $QT_VERSIONS_PATH ]]
  ...
  if [ "${VS_YEAR}" == "2019"]
  then
    export QT_ALT_VS_YEAR="2017"
  else
    export QT_ALT_VS_YEAR=${VS_YEAR}
  fi
  tmp_qt_path=$(/usr/bin/find $(cygpath -m $QT_VERSIONS_PATH) -maxdepth 3 -type d \
  \( -wholename "*/qt*${QT_VERSION}*-vs${VS_YEAR}-${ARCH_NO_BIT}bit" -o \
     -wholename "*/qt*${QT_VERSION}*-vs${QT_ALT_VS_YEAR}-${ARCH_NO_BIT}bit" -o \
     -wholename "*/Qt${QT_VERSION}/${QT_VERSION}*/msvc${VS_YEAR}_${ARCH_NO_BIT}" -o \
     -wholename "*/Qt${QT_VERSION}/${QT_VERSION}*/msvc${QT_ALT_VS_YEAR}_${ARCH_NO_BIT}" -o \
     -wholename "*/Qt/${QT_VERSION}*/msvc${VS_YEAR}_${ARCH_NO_BIT}" -o \
     -wholename "*/Qt/${QT_VERSION}*/msvc${QT_ALT_VS_YEAR}_${ARCH_NO_BIT}" \) \
  | head -1)
  if [ ! -z "$tmp_qt_path" ]
  then
    export QT_ROOT="${tmp_qt_path}"
    echo "Found matching QT root dir at: $QT_ROOT"
  else
